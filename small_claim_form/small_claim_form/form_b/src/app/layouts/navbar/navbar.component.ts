...
export class NavbarComponent {
  ...
  file?: File;
  previousData: { form_B: FormB } = { form_B: null };
  ...
  handleFileInput(event: Event): void {
    this.previousData.form_B = this.navbarService.getFormB();
  ...
  processFileContent(): void {
    ...
    reader.onload = () => {
      try {
        ...
        this.setFormB(jsonData.form_B, true);
      } catch (error) {
  ...
  private async setFormB(data: FormB, openUploadResultModal: boolean) {
    try {
      ...
      console.error("Error while populating form B.\n\n" + error);

      // reset the form to the previous state
      this.setFormB(this.previousData.form_B, false);
      this.openUploadResultModal(false);
    }
  }
